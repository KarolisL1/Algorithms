...
function recursiveBinarySearch(val, arr) {
    ...
    if (arr[mid] > val) {
        // return recursiveBinarySearch(val, arr.slice(0, mid));
        for (var i = 0; i < arr[mid]; i++) {
            if (arr[i] === val) {
                return i;
            }
        }
    ...
    if (arr[mid] < val) {
        // return recursiveBinarySearch(val, arr.slice(mid + 1, arr.length));
...
console.log(recursiveBinarySearch(8, [1, 2, 3, 3, 4, 4, 5, 6, 7, 8, 8, 9, 10]));
// [1, 2], -2 ---> -1
// [1, 2], 4 ---> -1
// [1, 2], 1 ---> 0
// [1,2,2], 2 ---> 1
